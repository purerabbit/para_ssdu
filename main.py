...
from mri_tools import rA, rAtA, ifft2  #
...
parser.add_argument('--train-path', type=str, default='/home/liuchun/Desktop/parallel_02/data/knee_singlecoil_well_360.npz', help='path of training data')
parser.add_argument('--val-path', type=str, default='/home/liuchun/Desktop/parallel_02/data/knee_singlecoil_well_360.npz', help='path of validation data')
parser.add_argument('--test-path', type=str, default='/home/liuchun/Desktop/parallel_02/data/knee_singlecoil_well_360.npz', help='path of test data')
parser.add_argument('--u-mask-path', type=str, default='/home/liuchun/Desktop/parallel_02/mask/undersampling_mask/mask_8.00x_acs24.mat', help='undersampling mask')
parser.add_argument('--s-mask-up-path', type=str, default='/home/liuchun/Desktop/parallel_02/mask/selecting_mask/mask_2.00x_acs16.mat', help='selection mask in up network')
parser.add_argument('--s-mask-down-path', type=str, default='/home/liuchun/Desktop/parallel_02/mask/selecting_mask/mask_2.50x_acs16.mat', help='selection mask in down network')
...
def forward(mode, rank, model, dataloader, criterion, optimizer, log, args,epoch,writer):
    ...
    for iter_num, data_batch in enumerate(t):
        ...
        mask_net_up=mask_net_up.permute(0,3,1,2)
        mask_net_down=mask_net_down.permute(0,3,1,2)
        ...
        output_up, loss_layers_up, output_down, loss_layers_down = model(net_img_up.contiguous(), mask_net_up, net_img_down.contiguous(), mask_net_down)
        
       
        
        output_up, output_down = output_up .contiguous(), output_down .contiguous()
        output_up_kspace = ifft2(output_up)
        output_down_kspace = ifft2(output_down)
        ...
        if mode == 'train':
            ...
            psnr += psnr_slice(a/a.max(), b/b.max())            
            ssim += ssim_slice(a/a.max(), b/b.max())
        loss += batch_loss.item()
    loss /= len(dataloader)

    #tensorboard
    inimg= torch.abs(pseudo2complex(net_img_up))
    testimg= torch.abs(pseudo2complex(output_up))
     
    if mode =='val':
        writer.add_image('images/val_input', inimg, epoch)
    elif mode =='train':
        writer.add_image('images/train_input', inimg, epoch)
...
def solvers(rank, ngpus_per_node, args):
    ...
    #导入dataloader
    dataset=Dataset(data_path=args.test_path, u_mask_path=args.u_mask_path, s_mask_up_path=args.s_mask_up_path, s_mask_down_path=args.s_mask_down_path)
    ...
    if args.mode == 'test':
        ...
        with torch.no_grad():
            ...
            test_log = forward('test', rank, model, test_loader, criterion, optimizer, test_log, args,epoch,writer)
